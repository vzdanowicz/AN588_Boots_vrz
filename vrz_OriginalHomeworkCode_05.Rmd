...
for (i in 1:1000) {
  ...
  d_boots <- lm(log(Body_mass_female_mean) ~ log(HomeRange_km2), data = d_boots) #I log transformed my y ~ x here
...
mean_betas <- list(Mean.Slope = as.numeric(mean_slope), Mean.Intercept = as.numeric(mean_intercept))

#Bootstrapped Standard Deviation
sd_slope <- sd(sample_coef_slope)
sd_intercept <- sd(sample_coef_intercept)
sd_betas <- list(SD.Slope = as.numeric(sd_slope), SD.Intercept = as.numeric(sd_intercept))

#Bootstrapped Standard Error
se_slope <- se(sample_coef_slope)
se_intercept <- se(sample_coef_intercept)
se_betas <- list(SE.Slope = as.numeric(se_slope), SE.Intercept = as.numeric(se_intercept))

beta.averages <- cbind(mean_betas,sd_betas,se_betas)
beta.averages
```
